...
namespace DADataManager
{
    public static class ClientDatabaseManager
    {
        ...
        public static DateTime GetMinTime(string tblname)
        {
            ...
            var sql = "select `TickTime` from `" + str[str.Length - 1].ToUpper() +
        ...
        //todo
        public static DateTime GetMinTimeBar(string tblname)
        {
            MySqlDataReader reader = null;

            DateTime time = new DateTime();
            var str = tblname.Trim().Split('.');
            var sql = "select `BarTime` from `" + tblname +
                      "` order by  `BarTime` asc  limit 1;";
            reader = GetReaderBar(sql);
            if (reader != null)
            {
                while (reader.Read())
        ...
        public static DateTime GetMaxTimeBar(string tblname)
        {
            MySqlDataReader reader = null;
            ...
            try
            {
                ...
                if (reader == null)
                {
                 ...
                 .ToString("yyyy/MM/dd HH:mm:ss", CultureInfo.InvariantCulture);
                    sql = "ALTER TABLE `" + table + "` ADD `Expiration` DateTime null default '" + s + "';";
                    DoSql(sql);
                }
        ...
        public static int ReturnSesionIDinGroup(int sesionID, int groupID)
        {
            int _groupID = -1;
            var sql = "select `GroupID` from `" + TblSessionsForGroups + "` ";
            sql += "where `SessionID`=" + sesionID + " and `GroupID`=" + groupID;
            var reader = GetReader(sql);
            if (reader != null)
            {
                while (reader.Read())
                {
                    _groupID = Convert.ToInt32(reader.GetValue(0));
        ...
        public static bool DeleteSesion(int sessionID)
        {
            var sql1 = " DELETE FROM " + TblSessionsForGroups + " WHERE SessionId=" + sessionID + "; COMMIT;";
            var sql2 = " DELETE FROM " + TblSessions + " WHERE Id=" + sessionID + "; COMMIT;";
            return DoSql(sql1) && DoSql(sql2);
        }
        ...
        public static void AddSessionForGroup(int groupID, int sesionID)
        {
            var sql2 = "INSERT INTO " + TblSessionsForGroups + " (GroupId, SessionId) VALUES (";
            sql2 += groupID + " , " + sesionID + ");COMMIT;";
        ...
        public static void SetUserDetails(int userID, bool isAutocollect, bool isEmeilAfterFinish, int barStart,
            int barEnd)
        {
            var table = TblUserInfo;
            ...
            try
            {
                ...
                {
                    reader.Close();
                    sql = "insert ignore into " + table + "(`userID`,`Autocollect`,`EmeilFinish`,`BarStart`,`BarEnd`)";
                    sql += "values(" + userID + ", " + isAutocollect + ", " + isEmeilAfterFinish + ", " + barStart +
                           ", " + barEnd + ");COMMIT;";
                    DoSql(sql);
                }
                else
                {
                    ...
                    sql += "`Autocollect`=" + isAutocollect + ", ";
                    sql += "`EmeilFinish`=" + isEmeilAfterFinish + ", ";
                    sql += "`BarStart`=" + barStart + ", ";
                    sql += "`BarEnd`=" + barEnd + " ";
                    sql += "where `userID`=" + userID+"; COMMIT;";
                    DoSql(sql);
                }
        ...
        public static void GetUSerInfo(int userID, out bool autocollect, out bool emeilFinish, out int barStart,
            out int barEnd)
        {
            var table = TblUserInfo;
            var reader = GetReader("Select * from `" + table + "` where `userID`=" + userID);
            if(reader!=null)
                while (reader.Read())
                {

                    autocollect = Convert.ToBoolean(reader.GetValue(2));
                    emeilFinish = Convert.ToBoolean(reader.GetValue(3));
                    barStart = Convert.ToInt16(reader.GetValue(4));
                    barEnd = Convert.ToInt16(reader.GetValue(5));
                    reader.Close();
                    return;
                }
            autocollect = false;
        ...
        public static int GetBarEnd(int userID)
        {
            var table = TblUserInfo;
            int barEnd=3000;
            var reader = GetReader("Select `BarEnd` from `" + table + "` where `userID`=" + userID);
            if (reader != null)
                while (reader.Read())
                {

                    barEnd = Convert.ToInt16(reader.GetValue(0));
                    break;
        ...
        public static string GetTickTableFromSymbolMin(string symbolName)
        {
            //var str = symbolName.Trim().Split('.');
            //var tblStart= "T_" + str[str.Length - 1].ToUpper();
            var list = GetListOfTickTables(symbolName);
            int min = 999999;
            string minname="";
            try
            {
                ...
                {
                    string s = li.Substring(li.LastIndexOf('_') + 1, li.Length - li.LastIndexOf('_') - 1);
            ...
            {
                return list.First();//if we don't have week number at the end
            }
            return minname;

        }
        public static string GetTickTableFromSymbolMax(string symbolName)
        {
            //var str = symbolName.Trim().Split('.');
           // return "T_" + str[str.Length - 1].ToUpper();
        ...
        public static List<string> GetListOfBarTables(string symbolName)
        {
            ...
            var databaseName = GetDatabaseName();//todo

            var tablesList = new List<string>();
            var reader = GetReaderBar("SHOW TABLES FROM " + databaseName);//todo
            if (reader != null)
            {
                while (reader.Read())
                {
        ...
        public static List<string> GetListOfTickTables(string symbolName)
        {
            var str1 = symbolName.Trim().Split('.');
            var shortSymbol = str1[str1.Length - 1].ToUpper();
            var databaseName = GetDatabaseNameTick();//todo

            var tablesList = new List<string>();
            var reader = GetReaderHistorical("SHOW TABLES FROM " + databaseName);//todo
            if (reader != null)
            {
                while (reader.Read())
                {
                    var str = reader.GetString(0).ToUpper();
                    if ((str[0] == 't' && str[1] == '_') || (str[0] == 'T' && str[1] == '_'))
                        if (str.Contains(shortSymbol))
                            tablesList.Add(str);
                }
                reader.Close();
        ...
        {
            var connection = CurrentDbIsShared ? _connectionStringToShareDbBar : _connectionStringToLocalDbBar;//todo 2
        ...
        private static string GetDatabaseNameTick()
        {

            var connection = CurrentDbIsShared ? _connectionStringToShareDbHistorical : _connectionStringToLocalDbHistorical;//todo 2
