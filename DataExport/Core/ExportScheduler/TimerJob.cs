...
//		public IJobLog Log;
		...
		public void Execute(object sender, DateTime Begin, DateTime End, ExceptionEventHandler Error,string proFileName)
		{
			if (!Enabled)
				return;
			...
			{
				if (SyncronizedEvent)
                    _ExecuteHandler(sender, EventTime, Error,proFileName);
				else
					_ExecuteHandler.BeginInvoke(sender, EventTime, Error, null, null,null);
			}
		}

		private void ExecuteInternal(object sender, DateTime EventTime, ExceptionEventHandler Error,string profileName)
		{
			try 
			{
				TimerParameterSetter Setter = new TimerParameterSetter(EventTime, sender,ProfileName);
		...
		private delegate void ExecuteHandler(object sender, DateTime EventTime, ExceptionEventHandler Error,string ProfileName);
	...
	public class TimerParameterSetter : IParameterSetter
	{
        ...
        /// <param name="profileName">The string to pass to the unbound string parameters</param>
		public TimerParameterSetter(DateTime time, object sender,string profileName)
		{
			_time = time;
			_sender = sender;
